...
aboutTheData: '<p><strong>Accessible subway stations</strong><br> Locations are from the MTA Subway Stations dataset, filtered for stations marked as compliant with the Americans with Disabilities Act (ADA).</p> <p><strong>Walking distance</strong>:<br> We defined walking distance as 800 meters along the walking network (sourced from OpenStreetMaps), using shortest-path algorithms, like Dijkstra&rsquo;s, instead of straight-line buffers. This is roughly equal to a 10-15 minute walk.</p> <p><strong>Population estimates</strong><br> Population counts come from U.S. Census block groups. We used PLUTO building-level data to allocate block group populations more accurately, based on the share of residential units inside each walkable area.</p> <p><a href="">Download data</a>.</p>'  
---
