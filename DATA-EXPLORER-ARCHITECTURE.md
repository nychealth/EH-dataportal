...
This document describes the data explorer SPA built under `data-explorer-new/`. It covers every file, the data pipeline, state management, URL synchronization, interaction-by-interaction behavior, and improvement suggestions.
...
### 8.1 Encapsulate State in an Object

**Problem:** Five top-level globals (`IndicatorID`, `MeasureID`, `GeoType`, `TimePeriodID`, `overlay`) are read and written from many files. It's hard to track where mutations happen.

**Suggestion:** Group them into a single state object with controlled access.
...
### 8.2 Cancel In-Flight Data Loads

**Problem:** Rapid indicator switches can cause overlapping async pipelines with no cancellation.

**Suggestion:** Use an `AbortController` in `loadData` and abort the previous one when a new load starts.
...
const loadData = async (id) => {
    ...
    const response = await fetch(url, { signal: currentLoadController.signal });
    // ...
};
```

### 8.3 Explicitly Destroy DataTables

**Problem:** DataTables is re-initialized on every `renderTable` call without destroying the old instance.

**Suggestion:** Check for and destroy existing instances before re-initializing.
...
### 8.4 Avoid Duplicate DOM IDs

**Problem:** Mobile and desktop templates duplicate IDs like `measureOptionsDropdownButton`.

**Suggestion:** Use distinct IDs for mobile (`measureOptionsDropdownButton1`) and desktop (`measureOptionsDropdownButton2`), or switch to class-based selection throughout.

### 8.5 Extract Shared Default-Measure Logic

**Problem:** `setDefaultMapMeasure`, `setDefaultTrendMeasure`, `setDefaultLinksMeasure`, and `setDefaultDisparitiesMeasure` all contain the same priority chain (Age-adjusted rate → rate → Rate → Percent → etc.).

**Suggestion:** Extract to a single function.
...
const pickDefaultMeasure = (measures) => {
    ...
    for (const word of priority) {
        const match = measures.find(m => m.MeasurementType.includes(word));
...
### 8.6 Type-Safe URL Param Parsing

**Problem:** URL params are parsed with `parseFloat()` in some places and `Number()` in others. TimePeriodID arrives as a string from the URL but is compared with `==` in some places and `===` in others.

**Suggestion:** Centralize URL param extraction into a single function that returns typed values.

```js
const parseExplorerParams = (search) => {
    const p = new URLSearchParams(search);
    return {
        id:           p.has('id') ? Number(p.get('id')) : null,
        MeasureID:    p.has('MeasureID') ? Number(p.get('MeasureID')) : null,
        GeoType:      p.get('GeoType') || p.get('GeoTypeID') || null,
        TimePeriodID: p.has('TimePeriodID') ? Number(p.get('TimePeriodID')) : null,
        overlay:      p.get('overlay') || null
    };
};
```

### 8.7 Guard Bar Chart Against Empty Data

**Problem:** If `filteredMapData` is empty (e.g., geo/time combination with no data), `renderBar` tries to access `data[0]?.GeoType` and `metadata[0]?.MeasurementType`, which can produce an undefined-property error or an empty chart with no feedback.

**Suggestion:** Add an early return with a user-visible message.

```js
if (!data || data.length === 0) {
    document.getElementById('barHolder').innerHTML =
        '<p class="p-3 text-muted">No data available for the selected options.</p>';
    return;
}
```

### 8.8 Implement Trend and Correlate Renderers

**Problem:** `renderTrendChart` and `renderCorrelate` are stubs. The tabs can be clicked and the data is filtered, but nothing renders.

**Suggestion:** Implement Vega-Lite specs similar to `bar.js` for trends (line chart by time period) and correlates (scatter plot).

### 8.9 Add a Loading Indicator

**Problem:** Data fetches (indicator JSON, TopoJSON, geo lookup) can take seconds. During this time the user sees stale content or a blank map.

**Suggestion:** Show a spinner or progress bar in the map area and overlay pane during `loadIndicator`.

### 8.10 Reduce Global Variable Count

**Problem:** `global.js` declares ~80+ variables at module scope. Many are only used within a single function or file.

**Suggestion:** Move file-local variables inside their respective files or into the `renderMeasures()` closure. Only variables that genuinely need cross-file access should remain global.
