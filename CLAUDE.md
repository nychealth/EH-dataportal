...
- `npm run lint` — ESLint (`no-undef`) over `assets/js/data-explorer/` and `assets/js/nr-topic-spa.js`. `eslint.config.mjs` has one block per target. The 10 DE files share one global scope, so that block derives their shared globals at config-load time by scanning the directory; `no-undef` catches the undefined-name typos that scope is most prone to. `no-unused-vars` is intentionally omitted — it false-positives on the cross-file global pattern. Names injected from outside the directory (libraries, and the inline `<script>` blocks in `themes/dohmh/layouts/data-explorer/single.html`) are listed in `EXTERNAL_GLOBALS`. The NR block needs no scan — that file is self-contained, so only its externals are listed. **Adding a file to `eslint.config.mjs` does not put it in scope**; the `lint` script's argument list is what selects files, and the two must be changed together.
...
`smoke` and the characterization harness **reuse a running dev server, start one if none is running, and never stop a server they didn't start** (via `scripts/dev-server.mjs`). Set `DE_BASE_URL` to point them at a server on a non-default port/environment. If a `hugo` process is running but they can't find it on :8080/:1313, they abort with instructions rather than start a second server — a second server poisons the running one's fingerprint cache.
...
- **Topic SPA (Phase 2)** — `assets/js/nr-topic-spa.js` with `themes/dohmh/layouts/neighborhood-reports/nr-topic-spa.html`. Topic content files set `layout: nr-topic-spa` and an explicit `url`, giving topic-first URLs.
