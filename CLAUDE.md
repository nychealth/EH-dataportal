...
- For a page with substantial inline JS, externalize it to a per-page folder under `assets/js/` and load via `resources.Get` → `partial "short-fingerprint.html"` → fingerprinted `<script src integrity=...>` — see `themes/dohmh/layouts/data-explorer/single.html` for the working example. It is currently the only page using a per-page JS folder; the congestion-pricing report was the second, and it was deleted along with its content and templates. Keep scripts as classic (non-module) tags when they share global scope across files — load order matters and isn't enforced by tooling, so state it explicitly in a template comment.
