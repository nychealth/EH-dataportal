...
- `npm run lint` — ESLint (`no-undef`) over `assets/js/data-explorer/`. The 10 files there share one global scope, so the flat config (`eslint.config.mjs`) derives their shared globals at config-load time by scanning the directory; `no-undef` catches the undefined-name typos that scope is most prone to. `no-unused-vars` is intentionally omitted — it false-positives on the cross-file global pattern. Names injected from outside the directory (libraries, and the inline `<script>` blocks in `themes/dohmh/layouts/data-explorer/single.html`) are listed in `EXTERNAL_GLOBALS`.
