...
### Five ways a local check silently lies

- **The local EHDP-data mirror serves only the extensions IIS has a MIME mapping for, and answers 404.3 for the rest.** `.geojson` has no default mapping, so `geography/CD.geojson` 404'd while sitting on disk: the NDHR report map died on `local_prod`/`local_stage` alone, and `characterize:ndhr check` went red on 4 of 4 targets with a 404 that reads as absent data. `.json` and `.csv` are mapped and unaffected. Fixed by a staticContent mimeMap in that mirror's own web.config `[2026-09-15]` — but a 404 there has two causes, so read the substatus before concluding the mirror is stale: `puma2020_to_cd.csv` and `puma2010_to_subboro_cd.csv` really are absent, and only `scripts/ndhr-build-cdlist.mjs` reads them.
...
### Neighborhood Development Health Report (NDHR)

A prototype section on `feature-NDHR-prototype`, built to
[documents/ndhr-prototype-plan-2026-09-10.md](documents/ndhr-prototype-plan-2026-09-10.md). It is
the Neighborhood Reports presentation at **community district** geography, with its rows computed
in the browser from EHDP-data's own indicator files rather than from precomputed payloads
(`assets/js/report-data/normalize.js`, whose parity harness is under Guardrails above).

360 pages from `content/ndhr/`, four kinds, all routed by `layout` within
...
- **`nr-leaflet.html` must never load on an NDHR report page.** It declares `highlightFeature`,
  ...
  module — the same collision documented for the NR report page, with the same consequence: a
...
| `ndhr` | `ndhr` | Prototype. 354 of its 360 pages are generated by a content adapter — see above |
...
- **A page with no renderable layout is dropped in silence — exit 0, no warning, and it is still in the sitemap.** `themes/dohmh/layouts/_default/single.html` is a **zero-byte file**, tracked that way since the repository's first commit `[verified 2026-09-11: `git cat-file -s` reads 0 on both this branch and `production`]`, so a `kind: page` whose named layout does not exist falls back to nothing and writes no HTML. Branch pages are unaffected — `themes/dohmh/layouts/_default/list.html` has content. A new section's generated pages therefore appear in the built English sitemap and not on disk until its layouts land, and nothing in the build output says why. Count the pages you expect; a build that succeeds is not evidence they rendered.
