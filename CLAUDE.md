...
`npm run smoke` / `smoke:all` runs the site's JS and fails on console errors; `characterize:site`
diffs every page's rendered structure against a committed baseline. **Invoke the
`running-site-checks` skill before running or interpreting either** — it carries the commands, the
per-environment variants, the allowlist rules, the concurrency and re-capture caps, and the eight
ways a result can mislead you.

**Run smoke before merging anything that touches `head.html`, `baseof.html`, the header/footer
partials, or `assets/js/`.** That trigger is here rather than in the skill because it fires while
you are editing those files, not when you have decided to run a check.
