...
// this is to close the side panel on mobile view
...
function onLanguageChange() {
	...
	// for some reason the legend text does not change the first time the language changes so do it manually
	// note that the legend text is in a div with id legend-translated
	const el = iframe.contentDocument.querySelector('.esriLegendServiceLabel');
	// get translated text
	const translateDiv = document.getElementById('legend-translated');
	const translateText = translateDiv.innerText;
	if (el.innerText != translateText) {
		el.innerText = translateText;
	}
}
...
function onIframeLoad() {
	var _divClasses = ["action-node", "esriPopup", "esriMobileInfoView", "single-result-details", "query-tab-header", "query-result-action-button",
		"resultlayers-select-div","results-number", "results-container","dijit_form_Select_0", "query-tabs-container", "_20_panel", "about-content"];
	var iframe = document.getElementById("webappbld").getElementsByTagName("iframe")[0];
...
function addLanguageChangeListener() {
	const target = document.getElementById('popup-translated-0');
			...
			for (let mutation of mutationsList) {
				if (mutation.type === 'childList' || mutation.type === 'characterData') {
					if (typeof window.onLanguageChange === 'function') {
						if (_currentLanguage != getLanguage()) {
...
// method to get the iframe reference
function getIframe() {
