...
// Off-screen Leaflet map-export path for the print/export modal: composites the
// live choropleth/bubble map into a PNG. The modal shell and chart-export path
// live in print.js, which this file must load immediately before (see single.html).

// Map export: uses an off-screen Leaflet map with `L.canvas({ padding: 0 })` as the
// renderer. Call `setView()` before adding vector layers — adding layers first causes
// number-measure exports to silently fail.

// console.log(" >> print-map.js");
