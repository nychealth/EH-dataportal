...
| 18. Indicator row click target (post-review) | **DONE 2026-09-15** — not a task this plan opened; raised by the user against a live page. The whole `.card-header` now toggles its panel, where the `<button>` covered **34%** of it. **The dead area was two zones and only one was a decision**: the strategy cell is Task 8's recorded trade, but the 565x46 band beneath the button inside its own column was a side effect of that cell setting the row height while the `h3` stayed at content height, and nothing had recorded it. `h-100` closes the band; `forwardHeaderClick` in `app.js` covers the cell by routing the click rather than by widening the button, so **Task 8's split is intact** — the button's contents, and so its accessible name, are untouched. Two guards carry it: a click already on a control returns early, without which the button's own click bubbles up and toggles twice; a click while text is selected returns early, which is what keeps the strategy prose selectable. Task 8's comment in `cards.js` is corrected, its stated cost no longer being true | Browser on a `dev_prod` server at 1460px with `forwardHeaderClick` confirmed defined, so the reads are of the new file: button **565x54 -> 565x100**, dead band **46px -> 0**, **34% -> 63%** of the header. Strategy cell opens, former band opens, and the button toggles **once** and not twice. The selection guard is two arms at the **identical point** (1210, 835) with the selection the only variable — no selection opens, selection does not; a one-arm test would have passed on a panel that was closed anyway. Accessible name still ends at the tertile sentence, with the strategy phrase not inside the button. At 500px the cell stacks under the button, `h-100` is a no-op at 54=54, and the stacked cell still opens. After a map-driven district switch the rebuilt cards still forward, which is what the delegated binding claims. `npm run lint` 0, `npm run characterize:ndhr check` 4 of 4, `npm run smoke` 38 clean, `npm run docs-check` 0 | `0163e5dbbb` |

**Status as of 2026-09-15:** **All sixteen numbered tasks are done on `feature-NDHR-prototype`, and rows 17 and 18 were added after them** — work this plan did not open, recorded here because each changes what a page is. A review
