...
**Scope: `assets/js/data-explorer/` only.** All other browser-side JS in this repo
— `assets/js/nr-topic-spa.js`, `main.js`, `site.js`, and any inline template
script — follows `.claude/commands/js-development.md` instead.

The split is deliberate but temporary. The two documents agree on most things and
contradict each other on exactly two (`console.log` format, and whether comments
end in a period); scoping them by directory keeps either rule from reaching a file
the other governs. The goal remains **one** unified convention across the whole
site — see §5h of `documents/site-wide-audit-2026-06-27.md` for what unification
costs and why it is cheapest after `feature-new-data-explorer` merges.

Originally synthesized from the pre-refactor explorer (formatting preferences) and
`assets/js/data-explorer/` (organizational improvements made during the Copilot
refactor).
