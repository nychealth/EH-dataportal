...
- A new `scripts/` directory for dev tooling, and relocation of the existing harness into it.
- ESLint flat config over `assets/js/data-explorer/` only.
- Three npm scripts: `lint`, `characterize`, `smoke`.
- A shared dev-server module, `scripts/dev-server.mjs`.
- A console-error smoke script, `scripts/smoke-pages.mjs`.
...
**Behavioral impact:** none, except the four DOMPurify calls in §8. Everything else is config, dev tooling, file relocation, and documentation.

## 3. Relocation: `documents/` → `scripts/`

`CLAUDE.md` defines `documents/` as "Internal audits and technical write-ups." The characterization harness is an executable, not a write-up; it landed there only because it was created alongside the state-namespace design and plan docs it supported. Adding two more executables to the same directory would entrench the miscategorization, and `scripts/` is the conventional home for npm-invoked tooling.

Nothing is broken today — `documents/` is not published by Hugo and is not referenced in any config — so this is correctness of organization, done now because it is far cheaper before the npm scripts hardcode the paths than after.
...
The `de-` prefix is kept on the harness because it genuinely is DE-only. It is *not* used on the two new scripts: the smoke test's page list is deliberately site-wide (§6.1 covers home, data-features, neighborhood-reports, data-stories, take-action), and the dev-server module is not DE-specific either. A `de-` prefix on those would actively mislead.

**References that must be updated (live):**

- The harness's own `BASELINE_DIR` / `CURRENT_DIR` constants and its usage header comments.
- `.gitignore:81` (`documents/de-characterization-current/`).
- `data-explorer-fresh-audit-2026-07-13.md` lines 26, 187, 321.
- `CLAUDE.md`'s repo-structure block, which gains a `scripts/` entry.

**References that must NOT be updated (archival):**

`.superpowers/sdd/task-*-brief.md` and `-report.md`, and `data-explorer-state-namespace-plan-2026-07-10.md`, carry roughly forty path references between them. These are records of work as it was actually performed at the time. Rewriting paths inside them would falsify the record; they stay exactly as they are.
...
- `no-unused-vars` — configuration deferred to measurement, see 4.3.
...
### 4.3 The `no-unused-vars` open question
...
The first implementation step therefore installs ESLint, runs it with a minimal config, and reads the real output before configuring this rule. Two outcomes are acceptable:
...
## 5. Dev-server module (`scripts/dev-server.mjs`)
...
- **Spawn the documented command verbatim** — `hugo server --environment dev_stage --cleanDestinationDir --logLevel debug -p 8080`, exactly as CLAUDE.md specifies, so no assumption about Hugo's flag behavior is baked into the tooling.
...
## 6. Console-error smoke test (`scripts/smoke-pages.mjs`)
...
The repo currently has none. These are the first three:
...
`characterize` accepts `-- --check` and `-- --baseline`, which the existing script already parses. Neither `characterize` nor `smoke` requires any setup: both resolve a server through §5.
...
- A `scripts/` entry in the repo-structure block.
- A short **Guardrails** block under *Build and validation*: the three commands; that `characterize` and `smoke` reuse a running server, start one if none is running, and never stop a server they did not start; the `DE_BASE_URL` override and what the path-3 abort message means; and that `characterize -- --check` and `smoke` are expected before any Tier 2–4 merge.

`data-explorer-fresh-audit-2026-07-13.md` §4.5 gains an execution-status entry, and its three stale harness paths are corrected. `site-wide-audit-2026-06-27.md` gains the four deferred items from §2.

## 10. Staging and verification

Six commits, each independently green.

| # | Change | Verification |
|---|---|---|
| 0 | Create `scripts/`; `git mv` the harness and its baseline; update the live references in §3 | `node scripts/de-characterization.mjs --check` exits 0 from its new location, proving the constants and baseline path survived the move. |
| 1 | ESLint config, devDeps, `lint` script | `npm run lint` exits 0. This step also measures the `no-unused-vars` question in §4.3 and records the outcome. |
| 2 | `scripts/dev-server.mjs`; rewire the harness to it; `characterize` script | All four resolution paths exercised by hand: with `DE_BASE_URL` set; with a server already running; with nothing running; and with a server on 1313 — which **must abort, not spawn**. |
| 3 | `scripts/smoke-pages.mjs`, `smoke` script | `npm run smoke` exits 0, with every allowlist entry justified by an audit citation. |
