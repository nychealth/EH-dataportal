...
**Open:** **4.4** (parked until comparative user testing ends), plus a new gap it inherits — see below. **4.12 is fixed** as of 2026-07-30 on `feature-de-tier4.12-null-geotype` — the map drew nothing and threw on 40 of 282 indicators; it now draws an explicit gray "not mapped" state with a message and a highlight on the visualization bar. That **removes the crash but does not fully clear 4.4's gate**: compared side by side on `?id=26` (2026-07-30), the old explorer's Table checks **both** Citywide and Borough by default and renders 7 rows, while the new explorer defaults to Citywide alone and renders 3 — even though `Borough` is present in `aqTableTimesGeos` and the user can tick it manually. That difference predates 4.12 and is untouched by it; it is now the remaining "renders thinner than the old one" item for these indicators, and it is a **table-default** question, not a map one. New from that work: **§4.13**, an upstream metadata question (10 measures have no `Citywide` row to fall back on, 9 of them `NYHarbor`-only despite the geometry file existing) — no crash, not urgent; and **§4.14** from the branch review, the three map renderers having no stale-render token, deferred because the pre-existing half wants its own change. Still logged-not-scheduled: the **4.2 follow-up** (URL updates 29–151 ms after the click; the recommendation is to leave it alone) and **4.11** (`DE.state` written from 23 sites across 5 files; the namespace refactor fixed naming, not ownership). **4.10** closed 2026-07-27 — measured first, and the perf case died; shipped as a clarity change.
...
**Status: mixed, as of 2026-07-30.** Closed: 4.1 (+ its naming sweep), 4.2, 4.3, 4.5, 4.6, 4.7, 4.8, 4.9, 4.10, 4.12. **Open: §4.13** (upstream metadata question, no crash), plus the low-priority 4.2 follow-up and 4.11. **Deferred by decision: 4.4**, until comparative user testing ends — 4.12 removed its map crash, but the thinner default Table on those same indicators is still outstanding (see the at-a-glance block).
...
> **Measured 2026-07-30, side by side on `?id=26`.** The "fuller table" is specifically a *default filter* difference, and it survives this fix: the old explorer ticks both `Citywide` and `Borough` under Boundaries and renders 7 rows; the new one defaults `DE.table.selectedTableGeography` to a single geography and renders 3. `Borough` is present in `DE.lookups.aqTableTimesGeos` either way, so the data is there and the user can tick it — this is a defaults question in `renderMeasures`, not a data or metadata one. It predates 4.12, is untouched by it, and is the remaining reason these indicators read as thinner than on the old explorer. Whoever picks up 4.4 should decide it; it was left alone here rather than widened into a table change under a map finding.
...
### 4.14 The three map renderers have no stale-render token, so a slow geometry fetch can land on the wrong measure

**Status: open (raised 2026-07-30, from the 4.12 branch review). Not reproduced in a browser — derived from reading the render path.** Deferred deliberately: the aliasing half is pre-existing in all three renderers and wants its own change rather than a ride-along in a map-metadata fix.

`renderChoroplethMap`, `renderBubbleMap` and `renderUnmappedCitywide` all do their work inside `loadMapGeojson(...).then(...)`, and nothing between `renderCurrentView(true)` and those `.then`s carries a render token. `resetMapForRender()` runs synchronously at the start of each render, so a second render can begin — and complete — while the first one's geometry fetch is still in flight.

The SPA already has this pattern twice, which is what makes the omission worth fixing rather than arguing about: `indicatorLoadToken` (app.js, from 4.2's stale-load race) and `pendingTableOverlayToken` (app.js). A third instance would follow those.

What a late `.then` does when it lands after a newer render:

- reassigns `currentGeojsonLayer`, so whichever layer lost the race is no longer tracked and **no later `resetMapForRender` can remove it** — it stays on the map until the page is reloaded;
- adds its own geometry on top of the current render's;
- for the unmapped renderer specifically, opens "Data not mapped for this indicator" over a working choropleth — the exact defect `currentMap.closePopup()` was added to fix in 4.12, reachable again through the async path — and consumes `DE.map.unmappedHintPending`, so the hint is spent on a measure that *is* mapped and never appears when the user returns to one that isn't.

Most likely on `?id=73` (3 unmapped of 6 measures) on a cold cache, where `citywide.topo.json` has not yet been fetched: land on the unmapped default, then change measure before the fetch resolves. `loadOnce` caches per geotype for the session, so it is a first-visit window, not a permanent one — which is also why it is easy to miss in testing.

**How to verify before fixing:** hold the geometry fetch open with Playwright's `page.route` and switch measure inside the window — the method is recorded in the `project-browser-verification-methods` memory and was used for 4.2's race. Prove the stranded layer and the spent flag, then add the token.
