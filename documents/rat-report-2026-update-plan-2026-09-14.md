...
`scripts/rat-report-render-check.mjs` renders all 32 in Chromium and diffs the drawn table against
the dataset each chart was given. **9 slots fail on 64 data cells, and one hides 2 rows**
`[verified 2026-09-21: 32 slots, 1104 cells compared, 0 slots comparing 0 cells]`:

| What differs | Slots | Cells | Fails the check? |
|---|---|---|---|
| Wrong values from an inherited override | `table-6-1`…`table-6-5` | 44 | **Yes** |
| Wrong values from an inherited override | `table-2-4`, rows 0-2 | 3 | **Yes** |
| Negative sign dropped from a percentage | `table-2-1`, `table-2-2`, `table-2-3`, `table-2-4` | 17 | **Yes** |
| `Period` supplied for an empty header cell | 10 slots | 10 | No — wanted |
| Header casing or punctuation | `table-33-1`, `#17`, `table-5a-5` | 7 | No — and one *fixes* a known defect |
| 2 rows hidden behind the page-2 arrow | `table-2-1` | — | **Yes** — see the pagination finding |

64 data cells + 17 header cells = the whole difference; nothing is unaccounted for. **The check
fails on a cell that holds data and reports a header cell without failing** — the distinction is
positional, not an allowlist, so a data cell can never be quietly excused. On a
`horizontal-header: true` chart uploaded row 0 is heading text, which is where the useful overrides
live; on every other chart row 0 is the first data row and is scored as one.

Three differences are normalised away as formatting: thousands separators, a real minus (U+2212)
drawn where the CSV has a hyphen, and the `%` a percent-formatted column appends to a bare number.
That last one matters — `8` in the data is drawn as `8%` on 11 slots, and scoring those as defects
reported **77 false positives** on the first full run. A sign *present on one side and absent on the
other* is deliberately not normalised, which is what keeps the 17 dropped-sign cells visible.
...
node scripts/rat-report-render-check.mjs              # the gate; exit 0 is apply's precondition
node scripts/rat-report-render-check.mjs table-6-1    # one slot, while iterating on a fix
node scripts/rat-report-charts.mjs apply              # only once the gate exits 0
```

**The render check's own arms were exercised before it was trusted** `[verified 2026-09-21]`: a
flag, two arguments and an unknown slot each exit 2; `table-6-1` fails on its 9 known cells; `#1`
passes with 0; and `table-33-1` reports 3 header differences while still exiting 0, which is the
arm that proves the header/data split rather than the diff itself. Its row alignment is read from
`horizontal-header` rather than inferred — the drawn `<thead>` *is* uploaded row 0 when that flag
is true and a synthesised header when it is false, and comparing without that offset reported ~28
false defects per chart on a first version. It loads the charts one at a time at a 3.5s settle
each, so the settle alone is ~112s over 32; total wall time was not measured.
